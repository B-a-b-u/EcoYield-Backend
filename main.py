...
def load_models():
    global crop_model, label_encoder, fertilizer_model, soil_encoder, crop_encoder, tflite_interpreter, tflite_input_details, tflite_output_details
...
def preprocess_tflite_image(file_bytes, target_size=(224, 224)):
    image = Image.open(io.BytesIO(file_bytes)).convert("L")  # Grayscale
    image = image.resize(target_size)
    img_array = np.array(image).astype(np.float32) / 255.0
    img_array = img_array.reshape(1, target_size[0], target_size[1], 1)  # Add batch + channel
    return img_array
...
async def predict_nutrient_deficiency(file: UploadFile = File(...)):
    image_bytes = await file.read()
    img_array = preprocess_tflite_image(image_bytes)
